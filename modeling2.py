...
class MTATTClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        x_mis = torch.cat([att_mis, x[1]], 1)
        x_mis = self.dropoutmis(x_mis)
        x_cat = torch.cat([att_cat, x[1]], 1)
...
class VHMTClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        att_mis = torch.cat([att_hmis, att_v], 1)
        att_mis = self.dropoutmis(att_mis)
        att_cat = torch.cat([att_hcat, att_v], 1)
