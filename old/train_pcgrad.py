...
def train(base_model, classifier,iterator, optimizer, cat_criterion,scheduler):
    ...
    for data_input, label_input  in Bar(iterator):
        ...
        optimizer.pc_backward([misogyny_loss,category_loss])
        optimizer.step()
...
def train_full(config, train_loader, valid_loader):
    ...
    cat_criterion = losses.FocalLoss(class_num=8)#nn.CrossEntropyLoss(weight=weights).to(device)
    ...
    optimizer_ = AdamW(optimizer_grouped_parameters, lr=config["lr"])
    ...
    scheduler = get_linear_schedule_with_warmup(optimizer_,  num_warmup_steps=warmup_steps, num_training_steps=num_train_steps)
    optimizer = PCGrad(optimizer_, scheduler)
    # Train model
...
if __name__ == "__main__":
    ...
    f = open(f'results/MTLPGC_{config["cls"]}_{args.lm_pretrained}_FL_{str(config["lr"])}.txt', mode='w')
    ...
    for RANDOM_SEED in seeds:
        ...
        train_loader, valid_loader = preprocessing.loadTrainValData(path=args.path, batchsize=args.batch_size, size=0.25, num_worker= 1, pretraine_path=config['pretrained_path'], seed=RANDOM_SEED)
