...
def eval_full(config, valid_loader):
    ...
    base_model.load_state_dict(torch.load(f'./ckpts/best_basemodel_MIS_ArMI_{config["lm"]}_{config["cls"]}_{str(config["seed"])}_.pth'))
    classifier.load_state_dict(torch.load(f'./ckpts/best_classifier_MIS_ArMI_{config["lm"]}_{config["cls"]}_{str(config["seed"])}.pth'))
    classifier.to(device)
    base_model.to(device)

    all_misogyny_outputs = evaluate(base_model, classifier, valid_loader)
...
if __name__ == "__main__":
    ...
    all_misogyny_outputs = eval_full(config, valid_loader)

    df_sub = pd.DataFrame(columns=['tweet_id', 'misogyny'])
    df_sub['tweet_id'] = ids
    df_sub['misogyny'] = all_misogyny_outputs
    df_sub['misogyny'].replace(dict_misogyny, inplace=True)

    df_sub.to_csv('results/CS-UM6P_subtask1_run1.csv', index=False, header=True, sep='\t')
