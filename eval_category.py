...
def eval_full(config, valid_loader):
    ...
    base_model.load_state_dict(torch.load(f'./ckpts/best_basemodel_CAT_ArMI_{config["lm"]}_{config["cls"]}_{str(config["seed"])}.pth'))
    classifier.load_state_dict(torch.load(f'./ckpts/best_classifier_CAT_ArMI_{config["lm"]}_{config["cls"]}_{str(config["seed"])}.pth'))
    classifier.to(device)
    base_model.to(device)

    all_category_outputs = evaluate(base_model, classifier, valid_loader)
...
if __name__ == "__main__":
    ...
    all_category_outputs = eval_full(config, valid_loader)

    df_sub = pd.DataFrame(columns=['tweet_id', 'category'])
    df_sub['tweet_id'] = ids
    df_sub['category'] = all_category_outputs
    df_sub['category'].replace(dict_category, inplace=True)

    df_sub.to_csv('results/CS-UM6P_subtask2_run1.csv', index=False, header=True, sep='\t')
