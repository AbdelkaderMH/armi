...
def train(base_model, classifier,iterator, optimizer, cat_criterion,scheduler):
    ...
    for data_input, label_input  in Bar(iterator):
        ...
        category_logits= classifier(output)
    ...
    losses = {'Category': loss_category / len(iterator)}
...
def train_full(config, train_loader, valid_loader):
    ...
    if config['cls'] =='1':
        classifier = modeling.CLSClassifier(in_feature=base_model.output_num(), class_num=8).to(device)
    elif config['cls'] =='2':
        classifier = modeling.ATTClassifier(in_feature=base_model.output_num(), class_num=8).to(device)
    elif config['cls'] =='3':
        classifier = modeling.VHClassifier(in_feature=base_model.output_num(), class_num=8).to(device)
    else:
        classifier = modeling.CLSClassifier(in_feature=base_model.output_num(), class_num=8).to(device)
        config['cls'] = '1'
    ...
    #cat_criterion = losses.FocalLoss(class_num=8)
    ...
    for epoch in range(config['epochs']):
        ...
        val_loss = valid_losses['Category']
        ...
        if F1_cat > best_cat_F1:
            ...
            torch.save(base_model.state_dict(), f'./ckpts/best_basemodel_CAT_ArMI_+{config["lm"]}_{config["cls"]}_{str(config["seed"])}.pth')
            torch.save(classifier.state_dict(), f'./ckpts/best_classifier_CAT_ArMI_{config["lm"]}_{config["cls"]}_{str(config["seed"])}.pth')
    ...
    return best_cat_F1, best_cat_acc, best_val_loss
def seed_all(seed):
    if not seed:
        seed = 10
...
if __name__ == "__main__":
    ...
    f = open(f'results/Category_{config["cls"]}_{args.lm_pretrained}_{str(config["lr"])}.txt', mode='w')
    #seed_all(12345)
    ...
    for RANDOM_SEED in seeds:
        ...
        best_cat_F1, best_cat_acc, best_val_loss = train_full(config, train_loader, valid_loader)
        avg_f1_cat += best_cat_F1
        avg_acc_cat += best_cat_acc
        print(f'Val. F1 Category: {best_cat_F1 * 100:.2f}%   \t Total_loss: {best_val_loss}')
        f.write(f' seed: {RANDOM_SEED}  \t best_mis_F1: {best_cat_F1} \t best_mis_acc: {best_cat_acc} \t val_loss: {best_val_loss} \n')
