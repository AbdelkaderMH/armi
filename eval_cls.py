...
def evaluate(base_model, classifier, iterator):
    ...
    all_category_outputs = np.empty((0,8))

    all_misogyny_outputs = np.empty((0,1))
    ...
    with torch.no_grad():
        for data_input in Bar(iterator):
            ...
            all_category_outputs = np.vstack(
                (all_category_outputs, category_probs.squeeze().float().cpu().numpy()))
            all_misogyny_outputs = np.vstack(
                (all_misogyny_outputs, misogyny_probs.float().cpu().numpy()))
...
def eval_full(config, valid_loader):
    ...
    for cls in ['1','2','3']:
        ...
        misogyny_outputs, category_outputs = evaluate(base_model, classifier, valid_loader)
        all_misogyny_outputs.append(misogyny_outputs)
        all_category_outputs.append(category_outputs)

    category_preds = (1 / (3) ) * np.sum(all_category_outputs, axis=0)
    misogyny_preds = (1 / (3) ) * np.sum(all_misogyny_outputs, axis=0)

    pred_category_outputs = np.argmax(category_preds, axis=1)
    pred_misogyny_outputs = np.round(misogyny_preds)
    pred_category_outputs = utils.label_rule(pred_misogyny_outputs, pred_category_outputs)


    return pred_misogyny_outputs, pred_category_outputs
...
if __name__ == "__main__":
    ...
    df_sub.to_csv('results/CS-UM6P_dev_run3.csv', index=False, header=True, sep='\t')
